...
module.exports = function (isStandalone = false) {
  return function FileComponent(app) {
    ...
    app.registerTimer(async function saveChat() {
      // TODO: 定时清理public/uploads/temporary文件夹
      ...
      try {
        ...
        let list = await db.models.file_avatar.findAsync({attach_uuid: null, createAt: orm.lt(ltdate)});
        ...
        let list_f = await db.models.file_file.findAsync({is_expired: false, createAt: orm.lt(ltdate_f)});
        ...
        for(let fi of list_f) {
          let filename = fi.name;
          let size = fi.size;
          let isExistOther = await db.models.file_avatar.existsAsync({name: filename, is_expired: false});
          if(!isExistOther) {
